...
class DataGeneratorPadding(DataGeneratorBase):
    ...
    def init_queue(self):
        ...
        for (name, shape) in self.data_names_and_shapes:
            ...
            self.placeholders.append(tf.compat.v1.placeholder(types, shape, name='placeholder_' + name))

        self.queue = tf.queue.PaddingFIFOQueue(self.queue_size, queue_types, queue_shapes)
        self.enqueue = self.queue.enqueue(self.placeholders)
