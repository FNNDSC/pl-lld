...
def softmax_cross_entropy_with_logits(labels, logits, weights=None, data_format='channels_first'):
    ...
    loss = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=logits, axis=channel_index)
...
def generalized_dice_loss(labels, logits=None, logits_as_probability=None, data_format='channels_first', weights=None, weight_labels=True, squared=True, weight_epsilon=1e-08, epsilon=1e-08):
    ...
    if weight_labels:
        ...
        label_weights = 1 / (tf.reduce_sum(input_tensor=labels, axis=image_axes) ** 2 + weight_epsilon)
    else:
        label_weights = 1
    # GDL_b based on equation in reference paper
    numerator = tf.reduce_sum(input_tensor=label_weights * tf.reduce_sum(input_tensor=labels * logits_as_probability, axis=image_axes), axis=1)
    if squared:
        # square logits, no need to square labels, as they are either 0 or 1
        denominator = tf.reduce_sum(input_tensor=label_weights * tf.reduce_sum(input_tensor=labels + (logits_as_probability**2), axis=image_axes), axis=1)
    else:
        denominator = tf.reduce_sum(input_tensor=label_weights * tf.reduce_sum(input_tensor=labels + logits_as_probability, axis=image_axes), axis=1)
