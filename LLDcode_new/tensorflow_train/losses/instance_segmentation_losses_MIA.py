...
def cosine_embedding_single_instance_loss(embeddings, target_instances_mask, other_instances_mask, normalized_embeddings=True, term_1_2_normalization='individual', term_0_squared=False, term_1_squared=False, return_h_norm=False,  use_first_frame_for_mean=False, is_background=False, data_format='channels_first'):
    ...
    h_norm = tf.nn.l2_normalize(h, axis=embedding_axis) #, epsilon=1e-4)
    # l2_normalize embeddings -> needed for cos_simliarity
    if normalized_embeddings is None:
        embeddings = tf.nn.l2_normalize(embeddings, axis=embedding_axis)
    else:
        embeddings = normalized_embeddings
    # calculate cos_similarity with target mean embedding and all embeddings
    cos_similarity = tf.reduce_sum(input_tensor=h_norm * embeddings, axis=embedding_axis, keepdims=True)
    ...
    if term_1_2_normalization == 'individual':
        ...
        term_0 = tf.boolean_mask(tensor=term_0, mask=target_instances_mask)
        term_1 = tf.boolean_mask(tensor=term_1, mask=other_instances_mask)
...
def cosine_embedding_all_instances_loss(embeddings, instances, normalized_embeddings, term_1_2_normalization, term_0_squared, term_1_squared, return_h_norm, is_background, instances_used, data_format, parallel_iterations):
    ...
    i = tf.constant(0, tf.shape(input=instances).dtype)
    num_instances = tf.shape(input=instances)[instance_axis]
    ...
    def per_instance_loss(i, current_ta_term_0, current_ta_term_1, current_ta_term_2):
        ...
        current_terms = tf.cond(pred=tf.greater(tf.math.count_nonzero(target_instances_mask), 0),
                                ...
                                false_fn=lambda: (tf.zeros([0], dtype=embeddings.dtype), tf.zeros([0], dtype=embeddings.dtype)) if not return_h_norm else (tf.zeros([0], dtype=embeddings.dtype), tf.zeros([0], dtype=embeddings.dtype), tf.zeros([embeddings.shape[0]], dtype=embeddings.dtype)))
    ...
    _, ta_term_0_final, ta_term_1_final, ta_term_2_final = tf.while_loop(cond=cond, body=per_instance_loss, loop_vars=(i, ta_term_0, ta_term_1, ta_term_2), parallel_iterations=parallel_iterations, swap_memory=True)

    term_0 = tf.cond(pred=tf.equal(num_instances, tf.constant(0, num_instances.dtype)), true_fn=lambda: tf.zeros([0], dtype=embeddings.dtype), false_fn=lambda: ta_term_0_final.concat())
    term_1 = tf.cond(pred=tf.equal(num_instances, tf.constant(0, num_instances.dtype)), true_fn=lambda: tf.zeros([0], dtype=embeddings.dtype), false_fn=lambda: ta_term_1_final.concat())
    if return_h_norm:
        term_2 = tf.cond(pred=tf.equal(num_instances, tf.constant(0, num_instances.dtype)), true_fn=lambda: tf.zeros([0], dtype=embeddings.dtype), false_fn=lambda: ta_term_2_final.stack())
...
def cosine_embedding_all_instances_bitwise_loss(embeddings, instances_bitwise, normalized_embeddings, term_1_2_normalization, term_0_squared, term_1_squared, return_h_norm, is_background, instances_used, data_format, parallel_iterations):
    ...
    num_instances = tf.cast(most_significant_bit(tf.reduce_max(input_tensor=instances_bitwise)) / 2, tf.int32)
    ...
    term_1 = tf.cond(pred=tf.equal(num_instances, tf.constant(0, num_instances.dtype)), true_fn=lambda: tf.zeros([0], dtype=embeddings.dtype), false_fn=lambda: ta_term_1_final.concat())
    if return_h_norm:
        term_2 = tf.cond(pred=tf.equal(num_instances, tf.constant(0, num_instances.dtype)), true_fn=lambda: tf.zeros([0, embeddings.shape[0]], dtype=embeddings.dtype), false_fn=lambda: ta_term_2_final.stack())
        return term_0, term_1, term_2
    else:
        return term_0, term_1
...
def cosine_embedding_per_instance_loss(embeddings, instances, normalized_embeddings=True, bitwise_instances=False, term_1_2_normalization='individual', l=1.0, term_0_squared=False, term_1_squared=False, return_h_norm=False, is_background=False, instances_used=None, data_format='channels_first', parallel_iterations=8):
    ...
    with tf.compat.v1.name_scope('cosine_embedding_loss'):
...
def cosine_embedding_per_instance_batch_loss(embeddings, instances, normalized_embeddings=True, bitwise_instances=False, term_1_2_normalization='individual', l=1.0, l2=0.0, term_0_squared=False, term_1_squared=False, is_background=False, instances_used=None, data_format='channels_first', parallel_iterations=8):
    ...
    if len(loss_list_term_2) > 0:
        total_loss += l2 * save_reduce_mean(tf.linalg.norm(tensor=tf.concat(loss_list_term_2, axis=0), ord=1, axis=1))
    return total_loss
