...
def cosine_embedding_single_instance_loss(embeddings, target_instances_mask, other_instances_mask, embeddings_norm=None, normalize=False, l=1.0, term_1_squared=False, term_2_factor=0, use_first_frame_for_mean=False, data_format='channels_first'):
    ...
    if embeddings_norm is None:
        embeddings_norm = tf.nn.l2_normalize(embeddings, axis=embedding_axis)
    ...
    cos_similarity = tf.reduce_sum(input_tensor=h_norm * embeddings_norm, axis=embedding_axis, keepdims=True)
    ...
    term_2 = 0
    if term_2_factor > 0:
        instance_mask = tf.reduce_any(input_tensor=target_instances_mask, axis=image_axes, keepdims=True)
        term_2 = tf.norm(tensor=h_norm, ord=1, axis=embedding_axis, keepdims=True)
...
def cosine_embedding_per_instance_loss(embeddings, instances, normalize=False, l=1.0, term_1_squared=False, term_2_factor=0, data_format='channels_first', parallel_iterations=4):
    ...
    if len(embeddings.shape) == 5:
        instances_transposed = tf.expand_dims(tf.transpose(a=instances, perm=[1, 0, 2, 3, 4]), axis=2)
    else:
        instances_transposed = tf.expand_dims(tf.transpose(a=instances, perm=[1, 0, 2, 3]), axis=2)
    ...
    return tf.reduce_mean(input_tensor=loss_list)
