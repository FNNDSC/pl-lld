...
def create_summary_placeholder(name):
    ...
    """
    placeholder = tf.compat.v1.placeholder(tf.float32, name='summary_placeholder_' + name)
    summary = tf.compat.v1.summary.scalar(name, placeholder)
...
class LayerWeightInspector(object):
    ...
    def __init__(self, conv_layer_names, summary_folder):
        ...
        self.summary = tf.compat.v1.summary.merge(self.summaries)
        self.writer = tf.compat.v1.summary.FileWriter(summary_folder)
    ...
    def get_histogram_summaries(self, tensors):
        ...
        for tensor in tensors:
            summary = tf.compat.v1.summary.histogram(tensor.name, tensor)
