...
class RotationMatrixBuilderTf(object):
    ...
    def rotate_all_random(self):
        ...
        rotation_variables = tf.compat.v1.get_variable('rotations', [self.dim, self.dim], dtype=tf.float32, initializer=tf.compat.v1.initializers.random_uniform(0, 1), constraint=lambda x: tf.maximum(tf.minimum(1.0, x), -1.0))
        for i in range(self.dim):
            for j in range(i + 1, self.dim):
    ...
    def create(self):
        ...
        for plane in self.planes:
            ...
            rotation = tf.sparse.sparse_dense_matmul(new_rotation, old_rotation)
...
class RotationMatrixBuilderTfGramSchmidt(object):
    ...
    def create(self):
        vectors = tf.compat.v1.get_variable('unorthogonal_rotation', [self.dim, self.dim], dtype=tf.float32)
        # add batch dimension for matmul
        basis = tf.expand_dims(vectors[0,:]/tf.norm(tensor=vectors[0,:]),0)
        for i in range(1,vectors.get_shape()[0].value):
            ...
            basis = tf.concat([basis, w/tf.norm(tensor=w)],axis=0)
        return basis