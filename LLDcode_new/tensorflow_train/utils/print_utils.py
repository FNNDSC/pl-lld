...
def printable_initializer(initializer):
    if initializer == he_initializer:
        ...
        return '0'
    elif isinstance(initializer, tf.compat.v1.initializers.variance_scaling):
        return 'var({},{},{})'.format(initializer.scale, initializer.mode, initializer.distribution)
    elif isinstance(initializer, tf.compat.v1.initializers.truncated_normal):
        return 'norm({}±{})'.format(initializer.mean, initializer.stddev)
    elif isinstance(initializer, tf.compat.v1.initializers.constant):
