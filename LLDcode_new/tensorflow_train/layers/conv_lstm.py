...
class ConvLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    ...
    def call(self, x, state):
        ...
        if self._dropout_ratio > 0:
            h = tf.compat.v1.layers.dropout(h, self._dropout_ratio, training=self._is_training)

        x = tf.concat([x, h], axis=self._feature_axis)
        n = x.shape[self._feature_axis].value
        m = 4 * self._filters if self._filters > 1 else 4
        W = tf.compat.v1.get_variable('kernel', self._kernel + [n, m], regularizer=self._regularizer)
        y = tf.nn.convolution(input=x, filters=W, padding='SAME', data_format=self._data_format_tf)
        if self._normalization is None:
            b = tf.compat.v1.get_variable('bias', [m], initializer=tf.compat.v1.zeros_initializer())
            y = tf.nn.bias_add(y, b, data_format=self._data_format_tf)
        j, i, f, o = tf.split(y, 4, axis=self._feature_axis)

        if self._peephole:
            i += tf.compat.v1.get_variable('W_ci', c.shape[1:], regularizer=self._regularizer) * c
            f += tf.compat.v1.get_variable('W_cf', c.shape[1:], regularizer=self._regularizer) * c
        ...
        if self._peephole:
            o += tf.compat.v1.get_variable('W_co', c.shape[1:], regularizer=self._regularizer) * c
    ...
    def call(self, x, h):
        with tf.compat.v1.variable_scope('gates'):
            ...
            r = self._conv(inputs,
                           ...
                           kernel_initializer=tf.compat.v1.constant_initializer(0))
            u = self._conv(inputs,
                           self._filters,
                           self._kernel,
                           data_format=self._data_format,
