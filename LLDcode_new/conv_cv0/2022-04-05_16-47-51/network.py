...
def network_scn(input, num_landmarks, is_training, data_format='channels_first'):
    ...
    with tf.compat.v1.variable_scope('local_appearance'):
    ...
    with tf.compat.v1.variable_scope('spatial_configuration'):
    ...
    with tf.compat.v1.variable_scope('combination'):
...
def network_downsampling(input, num_landmarks, is_training, data_format='channels_first'):
    ...
    with tf.compat.v1.variable_scope('downsampling'):
        for i in range(num_levels):
            with tf.compat.v1.variable_scope('level' + str(i)):
...
def network_scn_mmwhs(input, num_landmarks, is_training, data_format='channels_first'):
    ...
    kernel_initializer = he_initializer
    activation = tf.nn.relu
    local_kernel_initializer = tf.compat.v1.truncated_normal_initializer(stddev=0.0001)
    local_activation = tf.nn.tanh
    spatial_kernel_initializer = tf.compat.v1.truncated_normal_initializer(stddev=0.0001)
    spatial_activation = None
    padding = 'reflect'
    with tf.compat.v1.variable_scope('unet'):
